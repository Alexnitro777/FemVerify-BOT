...
Required per guild: `roles.verified`, `roles.blacklist`, `roles.admin`, `roles.mod`, `channels.review`, `channels.appealReview`, `questionCategoryId`. Optional: `roles.roleTag`, `channels.welcome|decisions|appeal|tagLog|blacklistLog`.
...
### Permissions
`permissions.ts` is two-tier and reads roles from the passed `GuildConfig`. Slash commands: `commandAccessLevel(interaction, gc)` resolves `admin` (Discord Administrator perm **or** a `gc.roles.admin` role) vs `mod` (a `gc.roles.mod` role); `hasCommandAccess(interaction, gc, required)` lets admin do everything and mod do only `access: 'mod'` commands. Buttons call `isMod(interaction, gc)` themselves (Manage Roles perm, or any admin/mod role) since the router only auth-checks slash commands.

### Constraints to respect
- Discord modals allow **max 5 input fields** — `verifyQuestions` in `questions.ts` is already at the limit.
- Question channels are created under `gc.questionCategoryId` with explicit permission overwrites for the applicant + every admin/mod role (`[...new Set([...gc.roles.mod, ...gc.roles.admin])]`).
- `index.ts` requests intents `Guilds`, `GuildMembers`, `GuildInvites` and `Partials.GuildMember`; SIGINT/SIGTERM trigger a clean `client.destroy()` + `closeDb()` (the latter ends the `mysql2` pool).
