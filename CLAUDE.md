...
- **Per-guild config** — rows in the `guild_settings` table (keyed `(guildId, key)`), entered **directly via SQL/GUI** (no file, no seeder). Dotted keys: `roles.verified`, `roles.blacklist`, `roles.staff`, `roles.ststaff`, `roles.roleTag`, `channels.review`, `channels.appealReview`, `channels.welcome|decisions|appeal|tagLog|blacklistLog`, `questionCategoryId`. `roles.staff`/`roles.ststaff` are comma-separated lists. The third tier, **owner**, is **not** in the DB — it's a hardcoded `OWNER_IDS` array in `permissions.ts`.
...
Required per guild: `roles.verified`, `roles.blacklist`, `roles.staff`, `roles.ststaff`, `channels.review`, `channels.appealReview`, `questionCategoryId`. Optional: `roles.roleTag`, `channels.welcome|decisions|appeal|tagLog|blacklistLog`.
