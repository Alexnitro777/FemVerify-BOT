...
│   │   ├── registry.ts        Реестр обработчиков customId (точные + RegExp)
...
│   ├── memberJoin.ts          Единый пайплайн входа участника (ЧС / верификация / тег / способ входа / ЛС)
│   ├── welcomeDM.ts           Приветственное ЛС при первом входе на любой сервер проекта
...
│   ├── channels.ts            Удаление канала-вопроса (общий помощник)
│   ├── concurrency.ts         Ограничение параллелизма для фоновых проходов
...
4. **Настрой серверы.** Конфигурации в файле нет — настройки каждого сервера лежат в таблице `guild_settings` (по строке на каждый ключ). Шаги ниже повтори для **каждого** сервера, где работает бот.
    ...
    **4.4. Вставь строки в `guild_settings`** через SQL или GUI (phpMyAdmin / Adminer / DBeaver). Ключи — с точками; `roles.staff` и `roles.ststaff` — это списки ID ролей через запятую. Замени плейсхолдеры на свои ID:
    ...
    | `roles.staff` | да | Роли staff через запятую — анкеты и тег: `/анкеты`, `/тег`, кнопки решения по анкетам |
    ...
    **Уровни доступа** (строгий режим — доступ дают только эти роли и owner-ID, право Discord «Администратор» само по себе ничего не даёт):
    ...
    - **staff** (`roles.staff`) — анкеты и статистика тега: `/анкеты`, `/тег`, кнопки решения по анкетам, закрытие каналов-вопросов.
