...
Discord-бот для верификации участников через анкеты, с модерацией заявок и апелляциями. Дополнительно автоматически выдаёт роль за тег сервера (Server Tag). Команды — слеш-команды (`/верификация`, `/апелляция` — owner; `/анкеты`, `/история`, `/тег` — staff; `/амнистии`, `/выдатьчсп`, `/снятьчсп` — ststaff), данные хранятся во внешней **MySQL/MariaDB** (через `mysql2`). Бот **мультисерверный**: один процесс обслуживает все серверы, на которых он есть, а роли и каналы настраиваются для каждого сервера отдельно.
...
4. **Настрой серверы.** Конфигурации в файле нет — настройки каждого сервера лежат в таблице `guild_settings` (по строке на каждый ключ). Шаги ниже повтори для **каждого** сервера, где работает бот.
    ...
    **4.4. Вставь строки в `guild_settings`** через SQL или GUI (phpMyAdmin / Adminer / DBeaver). Ключи — с точками; `roles.staff` и `roles.ststaff` — это списки ID ролей через запятую. Замени плейсхолдеры на свои ID:
    ...
    ('ID_СЕРВЕРА', 'roles.blacklistZ',      'ID_РОЛИ'),
    ('ID_СЕРВЕРА', 'roles.blacklistA',      'ID_РОЛИ'),
    ...
    | `roles.blacklist` | да | Основная роль ЧС (ЧСП) |
    | `roles.blacklistZ` | нет | Роль ЧС знакомств (ЧСЗ) — для выбора типа в апелляции |
    | `roles.blacklistA` | нет | Роль ЧС администрации (ЧСА) — для выбора типа в апелляции |
    | `roles.blacklistSoft` | нет | Доп. роли ЧС через запятую — участники могут подавать апелляции (тип ЧС), но не отключаются с голосовых каналов |
    | `roles.staff` | да | Роли staff через запятую — анкеты, история и тег: `/анкеты`, `/история`, `/тег`, кнопки решения по анкетам |
    | `roles.ststaff` | да | Роли ststaff через запятую — всё от staff + апелляции (`/амнистии`) + `/выдатьчсп` / `/снятьчсп` |
    ...
    **Уровни доступа** (строгий режим — доступ дают только эти роли и owner-ID, право Discord «Администратор» само по себе ничего не даёт):
    ...
    - **staff** (`roles.staff`) — анкеты, просмотр истории и статистика тега: `/анкеты`, `/история`, `/тег`, кнопки решения по анкетам, закрытие каналов-вопросов.
    - **ststaff** (`roles.ststaff`) — всё, что staff, плюс апелляции (`/амнистии`, кнопки решения по апелляциям) и `/выдатьчсп` / `/снятьчсп`.
