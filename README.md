...
4. **Настрой серверы.** Конфигурации в файле нет — настройки каждого сервера лежат в таблице `guild_settings` (по строке на каждый ключ). Шаги ниже повтори для **каждого** сервера, где работает бот.
    ...
    **4.4. Вставь строки в `guild_settings`** через SQL или GUI (phpMyAdmin / Adminer / DBeaver). Ключи — с точками; `roles.admin` и `roles.mod` — это списки ID ролей через запятую. Замени плейсхолдеры на свои ID:
    ...
    | `roles.admin` | да | Роли админов через запятую — полный доступ к командам |
    | `roles.mod` | да | Роли модерации через запятую — доступ к mod-командам и кнопкам |
